...
sigma_e = [4.4, 7.8, 7.8]';  % heterogeneity in e cells
sigma_i = [2.5, 16.25, 16.25]';  % heterogeneity in i cells
...
alpha_e = [1, 1, 1]';        % time scale E units
alpha_i = [2, 2, 2]';        % time scale I units

b = 0.5;              % adaptation gain
...
rng('default'); 
a_12 = 1 / 2; a_21 = 1 / 2; a_13 = 1 / 2; a_31 = 1 / 2;
a_23 = 1 / 2; a_32 = 1 / 2;
A = zeros(nRegions, nRegions); 
A(1, 2) = a_12; A(2, 1) = a_21; A(1, 3) = a_13; A(3, 1) = a_31;
A(2, 3) = a_23; A(3, 2) = a_32;
...
title(sprintf('%c_e = %.2f and %c_i = %.2f', 963, sigma_e(2), 963, sigma_i(2)))
subplot(nRegions, 2, 5); plot(tVect, Ue_trial(3, :), 'LineWidth', 1.2); xlabel('time (ms)'); ylabel('potential (mV)')
title(sprintf('%c_e = %.2f and %c_i = %.2f', 963, sigma_e(3), 963, sigma_i(3)))
subplot(nRegions, 2, 6); plot(tVect, Ui_trial(3,:), 'LineWidth', 1.2); xlabel('time (ms)'); ylabel('potential (mV)')
title(sprintf('%c_e = %.2f and %c_i = %.2f', 963, sigma_e(3), 963, sigma_i(3)))
...
subplot(1, 3, 3);
plot(Ue_trial(3, :), Ui_trial(3, :), 'LineWidth', 1.2)
xlabel('U_e'); ylabel('U_i');
title('Region 03 - Phase Plot')
