...
plotGraph(P, ifSave=True, save_dir=save_dir, figsize = (15, 5))
...
for I in modAmp:
    ...
    for nK, K in enumerate(Karr):
        ...
        _cv = {
                ...
                "mean": np.zeros((nNodes, sigSteps, sigSteps)),
                "var_cv": np.zeros((sigSteps, sigSteps)),
                "var": np.zeros((nNodes, sigSteps, sigSteps))
        }
        ...
        for nse, _sige in enumerate(sigArr):
            ...
            for nsi, _sigi in enumerate(sigArr):
                ...
                _lyapunov['nonmod_nodes'][:, nse, nsi] = np.mean(_lyap[:, 1:], axis=0)
                _lyapunov['nonmod_nodes_var'][:, nse, nsi] = np.std(_lyap[:, 1:], axis=0)
                ...
                _cv["mean"][:, nse, nsi] = np.mean(_mean, axis=0)
                _cv["var"][nse, nsi] = np.mean(_var, axis=0)
