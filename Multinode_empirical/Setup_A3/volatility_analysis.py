...
save_str = datetime.datetime(2025, 1, 23)
...
P, ROI, ROItag = modelP(nNodes, nRealizations, data_root="../data", n_files = 88,  WD="High")
...
sigeArr = np.array([2.5, 6.92]); sigSteps = len(sigeArr)
sigiArr = np.array([3.97, 16.5]); sigSteps = len(sigiArr)
...
for I in modAmp:
    ...
    for nK, K in enumerate(Karr):
        ...
        _lyapunov = {
                        ...
                        "nonmod_nodes": np.zeros((nNodes - 1, sigSteps, sigSteps)),
                        "nonmod_nodes_var": np.zeros((nNodes - 1, sigSteps, sigSteps)),
                        "network": np.zeros((sigSteps, sigSteps))
                    }
        _cv = {
                "mean_cv": np.zeros((sigSteps, sigSteps)),
                "mean_mean": np.zeros((nNodes, sigSteps, sigSteps)),
                "var_mean":np.zeros((nNodes, sigSteps, sigSteps)),
                "var_cv": np.zeros((sigSteps, sigSteps)),
                "mean_var": np.zeros((nNodes, sigSteps, sigSteps)),
                "var_var": np.zeros((nNodes, sigSteps, sigSteps))
        }
        ...
        for nse, _sige in enumerate(sigeArr):
            ...
            for nsi, _sigi in enumerate(sigiArr):
                ...
                _lyap = np.zeros((nTrials, nNodes, T - t_start - 1))
                for j in range(nTrials):
                    _diff_mod = np.abs(np.diff(u[j, 0::2, t_start:], axis=1)) / (params['dt'] * 10)
                ...
                _lyapunov['nonmod_nodes'][:, nse, nsi] = np.mean(_meanlyap[:, 1:], axis=0)
                _lyapunov['nonmod_nodes_var'][:, nse, nsi] = np.std(_meanlyap[:, 1:], axis=0)
                _lyapunov['network'][nse, nsi] = np.mean(_lyap)
                
                # _cvmeasure, _mean, _var = CoeffVariance(u, t_start, t_end)
                # _cv["mean_cv"][nse, nsi] = np.mean(_cvmeasure)                  # Mean of CV
                # _cv["mean_mean"][:, nse, nsi] = np.mean(_mean, axis=0)          # Mean of mean activities of each node
                # _cv["var_mean"][:, nse, nsi] = np.std(_mean, axis=0)            # Std of mean activities of each node
                # _cv["mean_var"][:, nse, nsi] = np.mean(_var, axis=0)            # Mean of std activities of each node
                # _cv["var_var"][:, nse, nsi] = np.std(_var, axis=0)              # Std of std activities of each node
                # _cv["var_cv"][nse, nsi] = np.std(_cvmeasure)                    # Std of CV
