...
if __name__ == '__main__':    
    ...
    save_str = datetime.datetime(2025, 1, 7)
    ...
    # Karr = np.array([-0.8, -0.2, 0.2, 0.8])
    Karr = [float(sys.argv[1])]
    Ksteps = len(Karr)
    ...
    for nK, K in enumerate(Karr):
        params['Kglob'] = K
        print(f"Running evaluation for K = {K:.1f}")
        for nse, _sige in enumerate(sigeArr):
            ...
            for nsi, _sigi in enumerate(sigiArr):
                ...
                file_str_k = f"KGlob_{params['Kglob']:.2f}_sige_{params['sigma'][0]:.3f}_sigi_{params['sigma'][1]:.3f}"
                ...
                if os.path.isfile(file_name):
                    ...
                    if len(plot_fps['FPs']) == 0:
                        print("No fixed points were detected!!")
                        continue
