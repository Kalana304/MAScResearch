...
def euler_intergrate(u, init_point, **opts):
    """
        This function implements euler integration to solve the system of non-linear equations
        for 2 Node system, given the parameters.

        - parameters:
            u - (np.darray) an array of size (nTrials x 2 x T) to save the intergrated results
            init_points - (np.darray) an array of initial points of size (nTrials x 2) for the integration to start

        - returns:
            u -  (np.darray) an array of size (nTrials x 2 x T) having system dynamics
    """
    # common parameters
    beta = opts.get('beta')
    ...
    for k, y0 in enumerate(zip(x0_, x1_, x2_, x3_)):
        ...
        u[k, :, 0] = y0
        sige1 = opts.get('sig_e1')[k] if isinstance(opts.get('sig_e1'), list) else opts.get('sig_e1')
        sigi1 = opts.get('sig_i1')[k] if isinstance(opts.get('sig_i1'), list) else opts.get('sig_i1')
        sige2 = opts.get('sig_e2')[k] if isinstance(opts.get('sig_e2'), list) else opts.get('sig_e2')
        sigi2 = opts.get('sig_i2')[k] if isinstance(opts.get('sig_i2'), list) else opts.get('sig_i2')

        for t in range(opts.get('T') - 1):
            u[k, 0, t + 1] = u[k, 0, t] + opts.get('dt') / opts.get('tau_e') * \
                             (-u[k, 0, t] + opts.get('wee') * F(u[k, 0, t], beta, theta, gamma, sige1) + \
                              opts.get('wie') * F(u[k, 1, t], beta, theta, gamma, sigi1) + \
                              opts.get('i_e1') + opts.get('K') * u[k, 2, t]) + np.sqrt(2 * D0 * opts.get('dt')) * np.random.normal(0,1) / opts.get('Ne')

            u[k, 1, t + 1] = u[k, 1, t] + opts.get('dt') / opts.get('tau_i') * \
                             (-u[k, 1, t] + opts.get('wei') * F(u[k, 0, t], beta, theta, gamma, sige1) + \
                              opts.get('wii') * F(u[k, 1, t], beta, theta, gamma, sigi1) + opts.get('i_i1')) + np.sqrt(2 * D0 * opts.get('dt')) * np.random.normal(0,1) / opts.get('Ni')

            u[k, 2, t + 1] = u[k, 2, t] + opts.get('dt') / opts.get('tau_e') * \
                             (-u[k, 2, t] + opts.get('wee') * F(u[k, 2, t], beta, theta, gamma, sige2) + \
                              opts.get('wie') * F(u[k, 3, t], beta, theta, gamma, sigi2) + \
                              opts.get('i_e2') + opts.get('K') * u[k, 0, t]
                              ) + np.sqrt(2 * D0 * opts.get('dt')) * np.random.normal(0,1) / opts.get('Ne')

            u[k, 3, t + 1] = u[k, 3, t] + opts.get('dt') / opts.get('tau_i') * \
                             (-u[k, 3, t] + opts.get('wei') * F(u[k, 2, t], beta, theta, gamma, sige2) + \
                              opts.get('wii') * F(u[k, 3, t], beta, theta, gamma, sigi2) + opts.get('i_i2')
                              ) + np.sqrt(2 * D0 * opts.get('dt')) * np.random.normal(0,1) / opts.get('Ni')
    return u
