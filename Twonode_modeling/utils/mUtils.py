...
class SpectrumSynch(object):
    ...
    def GlobalSynch(self, psi_1, psi_2, prop_1, prop_2):
        ...
        for t in range(0, self.t2 - self.t1 - self.window_size, self.step_window):
            ...
            phase_sum = prop_1[t : t + self.window_size] * np.exp(1j * psi_1[t : t + self.window_size]) +\
                        prop_2[t : t + self.window_size] * np.exp(1j * psi_2[t : t + self.window_size])
    ...
    def MIPhase(self, psi_1, psi_2, prop_1, prop_2, n_neighbors = 4):
        ...
        for t in range(0, self.t2 - self.t1 - self.window_size, self.step_window):
            psi_1_window = prop_1[t : t + self.window_size] * psi_1[t : t + self.window_size]
            psi_2_window = prop_2[t : t + self.window_size] * psi_2[t : t + self.window_size]
            # weight_ = np.mean(prop_1[t : t + self.window_size]) * np.mean(prop_2[t : t + self.window_size])
            mi_phase.append(MI(psi_1_window.reshape(-1, 1), psi_2_window.reshape(-1, 1), n_neighbors = n_neighbors, discrete_features=False)[0])
    ...
    def MIAmplitude(self, amp_1, amp_2, prop_1, prop_2, n_neighbors = 4):
        ...
        for t in range(0, self.t2 - self.t1 - self.window_size, self.step_window):
            # weight_ = np.mean(prop_1[t : t + self.window_size]) * np.mean(prop_2[t : t + self.window_size])
            # mi_amp.append(weight_ * MI(amp_1[t : t + self.window_size].reshape(-1, 1), amp_2[t : t + self.window_size], n_neighbors = n_neighbors, discrete_features=False)[0])
            amp_1_window = prop_1[t : t + self.window_size] * amp_1[t : t + self.window_size]
            amp_2_window = prop_2[t : t + self.window_size] * amp_2[t : t + self.window_size]
            # weight_ = np.mean(prop_1[t : t + self.window_size]) * np.mean(prop_2[t : t + self.window_size])
            mi_amp.append(MI(amp_1_window.reshape(-1, 1), amp_2_window.reshape(-1, 1), n_neighbors = n_neighbors, discrete_features=False)[0])
