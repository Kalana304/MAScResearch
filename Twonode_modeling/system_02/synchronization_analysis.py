...
eval_sigma_e = np.concatenate((eval_sigma_e[0:3:3], eval_sigma_e[3::4])); sige_steps = len(eval_sigma_e)
eval_sigma_i = np.concatenate((eval_sigma_i[0:3:3], eval_sigma_i[3::4])); sigi_steps = len(eval_sigma_i)

k_min = -1; k_max = 1; k_steps = 10
eval_k = [-0.6, -0.2, 0.0, 0.2, 0.6]; k_steps = len(eval_k) # k_min + np.arange(0, k_steps) / k_steps * (k_max - k_min)
