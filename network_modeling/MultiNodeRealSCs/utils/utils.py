...
def modelP(N, nTrials, data_root, n_files = 88,  WD="High"):
    """
        This function reads the structural connectivity matrices from OSF dataset.
    ...
    # Reading ROI data from AAL

    AALMAp = pd.read_csv(os.path.join(data_root, "AAL_regions.csv"), sep=";")
    ...
    for q in range(nTrials):
        A = pd.read_csv(os.path.join(data_root, f"S{load_scs[q]:03d}.csv"), header=None).to_numpy()
        P[q] = A[ROI_sub, :][:, ROI_sub]
        ...
        ROItag_sub = ROItag_sub[DegreeInd]
        ROI_sub = ROI_sub[DegreeInd]

    return P, ROI_sub, ROItag_sub
