...
root_dir = "./results/fp/scene01"

save_str = datetime.datetime(2023, 12, 15)
...
save_file_name = os.path.join(save_dir, 'scene01_data_v3.pkl')
...
if os.path.isfile(save_file_name):
    ...
    for ie in np.arange(i_min, i_max + delta_i, delta_i):
        print(f"Evaluating for I = {ie:.3f}")

        params['i_e1'] = ie / params.get('gamma')

        for k in np.arange(0, 0 + delta_k, delta_k):
            params['K'] = k

            for sigma in np.arange(sig_min, sig_max, delta_sig):
