...
sigi_min = 2.5; sigi_max = 17.5; sigi_steps = 10; delta_sigi = (sigi_max - sigi_min) / sigi_steps
sige_min = 2.5; sige_max = 17.5; sige_steps = 20

eval_sigma_e1 = sige_min + np.arange(0, sige_steps) / (sige_steps - 1) * (sige_max - sige_min)
eval_sigma_e2 = sige_min + np.arange(0, sige_steps) / (sige_steps - 1) * (sige_max - sige_min)
...
for k in np.arange(k_min, k_max + delta_k, delta_k):
    ...
    for sigi in np.arange(sigi_min, sigi_max + delta_sigi, delta_sigi):
        params['sig_i1'] = params['sig_i2'] = sigi 
        file_name = f"lyap_K_{params['K']:.2f}_ie_{params['i_e1']:.3f}_sigi_{sigi:.3f}"
        ...
        if os.path.isfile(os.path.join(save_dir, file_name + '.pkl')):
            ...
            lyap_mats = {
                'n1_mean': np.zeros((sige_steps, sige_steps)),
                'n2_mean': np.zeros((sige_steps, sige_steps)),
                'n1_max': np.zeros((sige_steps, sige_steps)),
                'n2_max': np.zeros((sige_steps, sige_steps))
            }
            for ns1, sige_1 in enumerate(eval_sigma_e1):
                params['sig_e1'] = sige_1 
                tic = time.time()

                for ns2, sige_2 in enumerate(eval_sigma_e2):
