...
for nV, _sigvar in enumerate(sigVar):
    for nI, I in enumerate(iPert):
        ...
        for k_in, K in enumerate(eval_k):
            tic = time.time()
            params['K'] = K
            file_name = f"lyap_K_{params['K']:.2f}_ie_{params['i_e1']:.3f}_sigVar_{_sigvar:.2f}"
                ...
                for e1, sigma_e in enumerate(eval_sigma_e):
                    ...
                    for i1, sigma_i in enumerate(eval_sigma_i):
                        ...
                        for q in range(n_trials):
                            ...
                            sige2 = abs(np.random.normal(7.5, _sigvar, 1))
                            sigi2 = abs(np.random.normal(16.5, _sigvar, 1))
                        ...
                        for k in range(n_trials):
                            _diff = np.abs(np.diff(u[k, 0, t_start:]))
