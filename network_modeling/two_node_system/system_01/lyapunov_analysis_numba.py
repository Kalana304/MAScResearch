...
saveStr = datetime.datetime(2025, 1, 16)
...
def _simulateI( init_points, 
    ...
    results = np.zeros((kSteps, sigSteps))
    ...
    for k1, k in enumerate(kValues):
        for s1, sig in enumerate(sigValues):
            ...
            lyapVal = _calcLyapunov(u=u)
            results[k1, s1] = lyapVal
...
for nI, I in enumerate(iPert):
    ...
    saveFile = f"lyap_ie_{i_e1:.3f}_D0_{D0 * (gamma * gamma):.3f}_dt_{dt}ms"
