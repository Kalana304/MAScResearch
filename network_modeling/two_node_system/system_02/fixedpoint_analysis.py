...
save_file_name = os.path.join(save_dir, 'scene02_data_K0.pkl')
...
if os.path.isfile(save_file_name):
    ...
    for ie in np.arange(i_min, i_max, delta_i):
        ...
        for k in np.arange(0, 0 +  delta_k / 2, delta_k):
            params['K'] = k

            for sige in np.arange(sig_min, sig_max, delta_sig):
                params['sig_e1'] = params['sig_e2'] = sige 
                
                for sigi in np.arange(sig_min, sig_max, delta_sig):
