...
save_str = datetime.datetime(2024, 4, 1)
...
# setting variables
iPert = np.array([0.  , 0.15, 0.25, 0.35, 0.5])[::-1] # [0.0, 0.005, 0.05, 0.5]
...
eval_sigma_e = np.array([2.5, 7.5, 13.5]); sige_steps = len(eval_sigma_e)  # sige_min + np.arange(0, sige_steps) / (sige_steps - 1) * (sige_max - sige_min)
eval_sigma_i = np.array([4.4, 13.5, 16.5]); sigi_steps = len(eval_sigma_i)  # sigi_min + np.arange(0, sigi_steps) / (sigi_steps - 1) * (sigi_max - sigi_min)
